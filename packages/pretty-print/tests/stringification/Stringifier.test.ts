...
describe('PPStringifier', () => {
  ...
  describe('stringify (utilInspectLike)', () => {
    ...
    it('Stringifies a number', () => {
      assert.deepStrictEqual(PPStringifiedValue.toUnstyledStrings(stringify(42)), ['42']);
    });

    it('Stringifies a string', () => {
      assert.deepStrictEqual(PPStringifiedValue.toUnstyledStrings(stringify('hello')), ["'hello'"]);
    });

    it('Stringifies null', () => {
      assert.deepStrictEqual(PPStringifiedValue.toUnstyledStrings(stringify(null)), ['null']);
    });

    it('Stringifies undefined', () => {
      assert.deepStrictEqual(PPStringifiedValue.toUnstyledStrings(stringify(undefined)), [
        'undefined',
      ]);
    });

    it('Stringifies a simple object', () => {
      assert.deepStrictEqual(PPStringifiedValue.toUnstyledStrings(stringify({ a: 1 })), [
        '{ a: 1 }',
      ]);
    });

    it('Stringifies a simple array', () => {
      assert.deepStrictEqual(PPStringifiedValue.toUnstyledStrings(stringify([1, 2, 3])), [
        '[ 1, 2, 3 ]',
      ]);
    });
    ...
    it('Replaces nested objects beyond maxDepth with a tag', () => {
      ...
      assert.isTrue(result.includes('[Object]'));
    });
    ...
    it('Stringifies a Map', () => {
      ...
      assert.isTrue(result.includes('Map'));
    });
  });
});
