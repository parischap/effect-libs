...
const _bigDecimalExtractor =
  ...
  (
    input: string,
  ): Option.Option<{ value: BigDecimal.BigDecimal; match: string; sign: -1n | 1n; input: string }> =>
    Option.gen(function* () {
      const {
        match,
...
const _numberExtractor:(self: Type) =>
  (
    input: string,
  )=> Option.Option<{ value: BigDecimal.BigDecimal; match: string; input: string }> = flow(
  _bigDecimalExtractor,
  Function.compose(
    Option.flatMap(({ value, match, sign, input }) =>Option.gen(function* () {
      const valueAsNumber =  yield* CVReal.fromBigDecimalOption(value)
    })
      pipe(
        value,
        ...
        Option.map(
          flow(
            ...
            Tuple.make,
            Tuple.appendElement(match),
            Tuple.appendElement(input),
          ),
        ),
      ),
    ),
 ...
 * containing `parsedText` (the part of `text` that could be analyzed as representing a number) and
 * `value` (`parsedText` converted to a BigDecimal value). Otherwise, it returns a `none`. As
 * `BigDecimal`'s provide no possibility to distinguish `-0n` and `0n`, parsing '-0', '0', '+0' will
...
> = flow(
  _bigDecimalExtractor,
  Function.compose(
    Option.flatMap(({ value, match, sign }) =>
      pipe(
        value,
        CVReal.fromBigDecimalOption,
        Option.map(
 ...
 * @category Parsing
 */
export const toRealParser: MTypes.OneArgFunction<
  Type,
...
export const toThrowingRealParser =
  ...
  (text: string): CVReal.Type => {
    const parser = toRealParser(self);
    return pipe(
      text,
      parser,
