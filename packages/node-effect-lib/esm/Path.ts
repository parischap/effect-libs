...
export interface ServiceInterface {
	...
	/**
	 * Used to convert a path to an absolute path. If lastSegment isn't already absolute,
	 * previousSegments are prepended in right to left order, until an absolute path is found. If
	 * after using all previousSegments paths still no absolute path is found, the current working
	 * directory is used as well. The resulting path is normalized, and trailing slashes are removed
	...
	/**
	 * Used to convert an absolute path to a relative path. If to or from is the zero-length string,
