...
import type * as MFs from '@parischap/effect-lib/MFs';
...
export interface ServiceInterface {
  ...
  readonly readDirectoriesUpwardWhile: <E, R>(params: {
    ...
    MFs.Folderpath,
    PlatformError.PlatformError | E | Cause.NoSuchElementError,
    R
  >;
          ...
          ([nextSeeds, parents]) =>
            Effect.gen(function* () {
              ...
              const newParents = yield* pipe(
                ...
                Result.map(Array.appendAll(parents)),
              );
        ...
        pipe(
          ...
          Stream.mapEffect((currentPath) =>
            pipe(
              ...
              Effect.flatMap(flow(MTuple.of, MTuple.prependElement(currentPath), isTargetDir)),
              Effect.map(flow(MTuple.of, MTuple.prependElement(currentPath))),
            ),
          ),
          Stream.takeUntil(Tuple.get(1)),
          Stream.runLast,
