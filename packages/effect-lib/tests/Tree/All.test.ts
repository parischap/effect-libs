...
import { MMatch, MString, MTreeAll, MTreeNode, MTuple, MTypes } from '@parischap/effect-lib';
...
describe('MTree', () => {
  ...
  const testTree1 = pipe(nonCyclicalObject1, MTreeAll.unfold(unfoldObject));
  const testTree2 = pipe(nonCyclicalObject2, MTreeAll.unfold(unfoldObject));
  const testTree3 = pipe(cyclicalObject, MTreeAll.unfold(unfoldObject));
  ...
  it('moduleTag', () => {
    TestUtils.assertEquals(
      Option.some(MTreeAll.moduleTag),
  ...
  describe('fold', () => {
    it('Non-cyclical value 1', () => {
      TestUtils.strictEqual(
        pipe(testTree1, MTreeAll.fold({ foldNonLeaf, foldLeaf })),
        foldedTestTree1,
      );
    });
    ...
    it('Non-cyclical value 2', () => {
      TestUtils.strictEqual(
        pipe(testTree2, MTreeAll.fold({ foldNonLeaf, foldLeaf })),
    ...
    it('Cyclical value', () => {
      TestUtils.strictEqual(
        pipe(testTree3, MTreeAll.fold({ foldNonLeaf, foldLeaf })),
  ...
  it('mapAccum', () => {
    TestUtils.strictEqual(
      pipe(
        ...
        MTreeAll.fold({ foldNonLeaf, foldLeaf }),
      ),
