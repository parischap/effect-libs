...
import { MFunction, MMatch } from '@parischap-dev/effect-lib';
import { Number, Option, flow, pipe } from 'effect';
...
describe('MMatch', () => {
		...
		it('tryFunction', () => {
			const testMatch = pipe(
				...
				MMatch.tryFunction(
					flow(
						Option.some,
						Option.filter(MFunction.strictEquals(4)),
						Option.map(() => 'd')
					)
				),
				MMatch.whenIs(5, () => 'b'),
	...
	describe('Refinement matching', () => {
		...
		const isA = MFunction.is<TestEnum, TestEnum.A>(TestEnum.A);
		const isB = MFunction.is<TestEnum, TestEnum.B>(TestEnum.B);
		const isC = MFunction.is<TestEnum, TestEnum.C>(TestEnum.C);
