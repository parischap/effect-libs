/**
 * This module implements a `MTemplate` which is a model of a text that has always the same
 ...
 * valuable information. We call them `MTemplatePlaceholder`'s.
 *
 * " is a ", "-year-old " and "." are the immutable parts of the template. We call them
 * `MTemplateSeparator`'s.
 *
 ...
 *
 * Once you have created a `MTemplate`, you must feed it to `MString.templateFormat` or
 * `MString.templateParse`.
 *
...
import * as internalTemplateParts from './internal/Template/TemplateParts.js';
 ...
 */
export const moduleTag = '@parischap/effect-lib/Template/';
const TypeId: unique symbol = Symbol.for(moduleTag) as TypeId;
type TypeId = typeof TypeId;

/**
 * Type that represents a MTemplate
 *
 * @category Models
 */
export class Type<out PlaceholderTypes extends MTypes.Object> extends MData.Class {
  ...
  readonly templateParts: internalTemplateParts.Type;
  ...
  /** Static constructor */
  static make<const PS extends internalTemplateParts.Type>(
    templateParts: PS,
  ): Type<internalTemplateParts.ToPlaceHolderTypes<PS>> {
    return new Type({
      syntheticDescription: internalTemplateParts.getSyntheticDescription(templateParts),
      placeholderDescription: internalTemplateParts.getPlaceholderDescription(templateParts),
      templateParts,
    });
  }
 ...
 */
export const make = <const PS extends internalTemplateParts.Type>(...templateParts: PS) =>
...
) => internalTemplateParts.Type = Struct.get('templateParts');
