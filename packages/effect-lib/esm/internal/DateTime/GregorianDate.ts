 ...
 */
export const moduleTag = '@parischap/effect-lib/internal/DateTime/GregorianDate/';
...
/**
 * Type that represents a GregorianDate
 *
 * @category Models
 */
...
const makeWithInternals = (params: Omit<MTypes.Data<Type>, 'daysInMonth'>): Type =>
  make({
    ...params,
 ...
 * `monthDay` as `self`. Returns a failure of an error otherwise. `year` must be an integer
 * comprised in the range [MIN_FULL_YEAR, MAX_FULL_YEAR]. If `self` represents a 29th of february,
 ...
 * `monthDay` as `self`. Returns a failure of an error otherwise. `month` must be an integer greater
    ...
    Result.gen(function* () {
      ...
      const monthDay = yield* pipe(
        ...
        Result.liftPredicate(
          Predicate.or(
            Number.isLessThanOrEqualTo(28),
            Number.isLessThanOrEqualTo(getNumberOfDaysInMonth(validatedMonth)(self)),
          ),
          (selfMonthDay) =>
            new MInputError.Type({
 ...
 * and `month` as `self`. Returns a failure of an error otherwise. `monthDay` must be an integer
 ...
 * `year` as `self`. Returns a failure of an error otherwise. `ordinalDay` must be an integer
 * greater than or equal to 1 and less than or equal to the number of days in the current year
 *
 * @category Setters
 */
