 ...
 */
export const moduleTag = '@parischap/effect-lib/internal/DateTime/IsoDate/';
...
/**
 * Type that represents an IsoDate
 *
 * @category Models
 */
 ...
 * `weekday` as `self`. Returns a failure of an error otherwise. `year` must be an integer comprised
 * in the range [MIN_FULL_YEAR, MAX_FULL_YEAR]. If the isoWeek of `self` is equal to 53, `year` must
 * be a long year.
 *
 * @category Setters
 */
    ...
    Result.gen(function* () {
      ...
      return yield* pipe(
        ...
        Result.liftPredicate(
          Predicate.or(yearIsLong, flow(getIsoWeek, Number.isLessThan(53))),
          () =>
 ...
 * `weekday` as `self`. Returns a failure of an error otherwise. `isoWeek` must be an integer
 ...
 * `isoWeek` as `self`. Returns a failure of an error otherwise. `weekday` must be an integer
 * greater than or equal to 1 (monday) and less than or equal to 7 (sunday).
 *
 * @category Setters
 */
