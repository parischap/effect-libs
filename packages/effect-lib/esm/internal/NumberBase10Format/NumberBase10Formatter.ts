 ...
 */
export const moduleTag = '@parischap/effect-lib/internal/NumberBase10Format/NumberBase10Formatter/';
const TypeId: unique symbol = Symbol.for(moduleTag) as TypeId;
type TypeId = typeof TypeId;

/**
 * Type that represents a NumberBase10Formatter
 *
 * @category Models
 */
export class Type extends MData.Class {
  ...
  /** Class constructor */
  constructor(numberFormat: MNumberBase10Format.Type) {
    super();
    this.description = `${MNumberBase10Format.toDescription(numberFormat)} formatter`;
    const rounder =
      numberFormat.maximumFractionalDigits === Infinity
        ? Function.identity
        : MBigDecimal.round(numberFormat.maximumFractionalDigits, numberFormat.roundingOption);
    const signFormatter = SignFormatter.fromSignDisplayOption(numberFormat.signDisplayOption);
    const mantissaAdjuster = ScientificNotationMantissaAdjuster.fromScientificNotationOption(
    ...
    const fractionalSeparatorPrepender = internal.prepend(numberFormat.fractionalSeparator);
    ...
    this.format = (number) =>
      Option.gen(function* () {
        ...
        const fractionalPartString = pipe(
          ...
          Option.map(internal.fromNonNullablePrimitive),
          Option.getOrElse(MFunction.constEmptyString),
        ...
        const integerPartString = pipe(
          ...
          MFunction.fIfTrue({
            ...
            f: flow(
              internal.splitEquallyRestAtStart(MRegExpString.DIGIT_GROUP_SIZE),
        ...
        const exponentString = pipe(
          exponent,
          Option.map(flow(internal.fromNumber(10), internal.prepend(eNotationChar))),
          Option.getOrElse(MFunction.constEmptyString),
        );
...
/**
 * Constructor of a `NumberBase10Formatter` from a `MNumberBase10Format`
 *
 * @category Constructors
 */
export const fromFormat = (format: MNumberBase10Format.Type) => new Type(format);
