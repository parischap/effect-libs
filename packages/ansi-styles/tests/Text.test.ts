...
describe('ASText', () => {
  ...
  it('concat', () => {
    TestUtils.strictEqual(ASText.toUnstyledString(ASText.concat(foo, bar, boldRedFoo)), 'foobarfoo');
  });
  ...
  it('surround', () => {
    TestUtils.strictEqual(ASText.toUnstyledString(bar.pipe(ASText.surround(foo, foo))), 'foobarfoo');
  });
