...
export class Type extends MEquivalenceBasedEqualityData.Class {
  ...
  [Hash.symbol](): number {
    return pipe(this.uniStyledTexts, Hash.array, Hash.combine(TypeIdHash));
  }
...
const _equivalence = Array.makeEquivalence(ASUnistyledText.equivalence);
/**
 * Equivalence
 *
...
const _haveSameTextequivalence = Array.makeEquivalence(ASUnistyledText.haveSameText);
/**
 ...
 * @category Utils
 */
export const surround = (prefix: Type, suffix: Type): MTypes.OneArgFunction<Type> =>
  flow(prepend(prefix), append(suffix));
...
export const appendIfNotEmpty =
  (that: Type): MTypes.OneArgFunction<Type> =>
  (self) =>
    isEmpty(self) ? self : concat(self, that);
...
export const prependIfNotEmpty =
  (that: Type): MTypes.OneArgFunction<Type> =>
  (self) =>
    isEmpty(self) ? self : concat(that, self);
...
export const surroundIfNotEmpty =
  (prefix: Type, suffix: Type): MTypes.OneArgFunction<Type> =>
  (self) =>
    isEmpty(self) ? self : concat(prefix, self, suffix);
...
export const join =
  ...
  (arr: ReadonlyArray<Type>): Type =>
    concat(...Array.intersperse(arr, self));
 ...
 * @category Utils
 */
export const removeEmptyAndJoin = (sep: Type): MTypes.OneArgFunction<Iterable<Type>, Type> =>
  Array.reduce(empty, (acc, elem) =>
    isEmpty(acc) ? elem : isEmpty(elem) ? acc : concat(acc, sep, elem),
  );
